...
def _insert_mock_data(cursor):
    ...
    for i in range(180, 0, -1):
        ...
        for _ in range(random.randint(1, 3)):
            ...
            tokens = random.randint(1000, 50000) if skill == 'ai-compress' else random.randint(100, 5000)
...
def get_stats():
    ...
    c.execute("SELECT SUM(revenue_generated) FROM events WHERE timestamp >= ?", (first_day_timestamp,))
    ...
    for i in range(5, -1, -1):
        ...
        c.execute("SELECT SUM(revenue_generated) FROM events WHERE timestamp >= ? AND timestamp < ?", (start_ts, end_ts))

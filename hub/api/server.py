...
def fetch_trending_skills():
    ...
    try:
        req = urllib.request.Request(
            "https://www.skills.sh/trending", headers={"User-Agent": "Mozilla/5.0"}
        )
                    ...
                    skills.append(
                        {
                            ...
                            "icon": "📦",
                            "desc": f"Trending skill developed by {authors[i]}"
                            if i < len(authors)
                            else "Trending AI Skill",
                            "rating": round(
                                4.0 + (i * 0.1), 1
                            ),  # mock rating based on position
                        }
                    )
        ...
        if self.path == "/api/quota":
            ...
            try:
                ...
                bin_path = os.path.abspath(
                    os.path.join(
                        os.path.dirname(__file__), "..", "..", "bin", "ai-quota"
                    )
                )
                # Added timeout=5 to prevent hanging if the script stalls
                result = subprocess.run(
                    [bin_path, "--json"], capture_output=True, text=True, timeout=5
                )
                if result.returncode == 0:
                    self.wfile.write(result.stdout.encode("utf-8"))
                else:
                    self.wfile.write(
                        json.dumps({"error": "Failed to run ai-quota"}).encode("utf-8")
                    )
                ...
                self.wfile.write(
                    json.dumps(
                        {
                            "status": "error",
                            "message": "Il comando ai-quota ha impiegato troppo tempo per rispondere.",
                        }
                    ).encode("utf-8")
                )
            except Exception as e:

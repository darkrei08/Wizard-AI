...
if ($args.Count -eq 0) {
    Write-Host 'ai-usage — Track Gemini CLI token usage and session costs'
    Write-Host ''
    Write-Host 'Usage:'
    Write-Host '  ai-usage                   Show current session usage'
    Write-Host '  ai-usage --today           Consumption today'
    Write-Host '  ai-usage --rolling-24h     Last 24 hours'
    Write-Host '  ai-usage --quota           Quota status'
    Write-Host '  ai-usage --history         Full usage history'
    Write-Host ''

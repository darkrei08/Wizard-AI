...
function Show-Help {
    Write-Host 'wz-ai graph — Graphify knowledge graph builder'
    Write-Host ''
    Write-Host 'Usage:'
    Write-Host '  wz-ai graph [path]              Build graph for path (default: current dir)'
    Write-Host '  wz-ai graph <github-url>        Clone repo and build graph'
    Write-Host '  wz-ai graph query "<question>"  Query existing graph'
    Write-Host '  wz-ai graph update              Incremental update (only changed files)'
    Write-Host '  wz-ai graph open                Open graph.html in browser'
    Write-Host '  wz-ai graph report              Show GRAPH_REPORT.md'
    Write-Host '  wz-ai graph wiki                Build agent-crawlable wiki'
    Write-Host ''
    Write-Host 'Options:'
    ...
    Write-Host ''
    Write-Host 'Examples:'
    Write-Host '  wz-ai graph .                   # Current directory'
    Write-Host '  wz-ai graph C:\projects\myapp   # Specific project'
    Write-Host '  wz-ai graph https://github.com/user/repo'
    Write-Host '  wz-ai graph query "how does auth work?"'
    Write-Host '  wz-ai graph open                # Open in browser'
}
...
try {
    ...
    if ($tcpConnection.Connected) {
        ...
        $CockpitArgs = @("--backend", "openai", "--model", "gemini-3.1-pro-high", "--max-concurrency", "1", "--token-budget", "25000", "--max-workers", "4")
    }
} catch {
...
switch ($First) {
    'open' {
        ...
        if (-not $GraphHtml) {
            Write-Host "[X] No graph.html found. Run 'wz-ai graph .' first." -ForegroundColor Red
            exit 1
        }
    ...
    'report' {
        $Report = Get-ChildItem -Path . -Recurse -Filter 'GRAPH_REPORT.md' -ErrorAction SilentlyContinue |
            ...
            Select-Object -First 1
        if (-not $Report) {
            Write-Host "[X] No GRAPH_REPORT.md found. Run 'wz-ai graph .' first." -ForegroundColor Red
            exit 1
        }

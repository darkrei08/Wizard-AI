...
3. **Hybrid Context Pipeline (`pi.dev + sqz + lean-ctx`)**: Pipe CLI and compiler outputs directly through `sqz` before feeding them to the model, or use `ai-lean-ctx` to drop stale conversation history while preserving core `MEMORY.md` state.

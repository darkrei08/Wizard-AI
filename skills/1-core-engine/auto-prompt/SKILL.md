...
- **Cost-Efficient Routing**: The `ai-prompt` wrapper uses a dual-level fallback strategy via Simon Willison's `llm` CLI. It uses fast, cheap models (like `gemini-1.5-flash` or `claude-3-haiku`) for the rewrite, switching to larger context models only when necessary.

## Usage

```bash
# Basic usage with raw text
ai-prompt "ho un bug nel carrello, non si aggiorna il totale. file: cart.js. dimmi dove sbaglio."

# Usage with a file
ai-prompt -f raw_idea.txt

# Usage for highly ambiguous/critical tasks (forces parallel/advanced models)
ai-prompt "progetta l'intera architettura del nuovo DB partendo da zero" --critical
```

## Architecture

1. **CLI Wrapper (`bin/ai-prompt`)**: The entry point. It counts tokens to determine the best model strategy.
...
2. Run `ai-prompt "user request" > structured_prompt.xml`.

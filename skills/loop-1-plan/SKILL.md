...
- **`gentle-ai`**: *Quando usarla:* All'inizio di un nuovo progetto o refactoring per configurare l'ecosistema SDD, i prompt di sistema e l'ambiente dell'agente.
- **`engram`**: *Quando usarla:* Per recuperare decisioni architetturali o preferenze utente persistite nel tempo, sostituendo claude-mem.
...
- **`system-design-101`**: *Quando usarla:* Per convalidare l'architettura, i protocolli (REST, gRPC), le scelte del database o le strategie di caching rispetto agli standard di settore.

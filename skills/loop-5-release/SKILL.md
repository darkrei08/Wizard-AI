...
- **`repodocs`**: *Quando usarla:* Quando le modifiche architetturali o la nuova API pubblica necessitano di rigenerare la documentazione del repository (wiki codebase). *Cosa fa:* Scansiona il codice e scrive/aggiorna pagine markdown referenziate e file `wiki.html`. Esegui solo se la struttura o le interfacce primarie sono cambiate.
- **`blume`**: *Quando usarla:* Quando il progetto deve pubblicare documentazione leggibile da umani e agenti (non solo il knowledge graph interno) E il sito docs è già presente (`blume.config.ts` o cartella `.blume/`). *Cosa fa:* Trasforma la cartella markdown/MDX in un sito docs di produzione (`blume build`) con ricerca locale, i18n, reference OpenAPI, `llms.txt` e MCP server. Esegui sempre `blume validate` prima del rilascio: `blume build` da solo non controlla i link. Distinzione: `repodocs` genera i testi markdown partendo dal codice, `blume` prende quei testi e li pubblica in un sito web.
...
- Se il progetto include uno script di release automatizzato nel `package.json` (come in Wizard-AI):
  ```bash
  npm run release
  ```
  *(Questo script si occuperà in un solo colpo di: aggiornare la versione patch, fare il push su GitHub, creare la GitHub Release con le note, e pubblicare su NPM).*

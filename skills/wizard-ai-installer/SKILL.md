...
| `~/.wizard-ai/` | Local Python tool installs, venvs, and operational cloned repos |
| `$WIZARD_AI_DIR/resources/Repositories-Skills/` | Local clones of external repos used ONLY for reference (repodocs, blume) |
...
- **GitHub Repository (Operational)**: Clone it to `~/.wizard-ai/<repo-name>` if it's an operational tool that requires execution.
- **GitHub Repository (Reference/Docs)**: Clone it to `$WIZARD_AI_DIR/resources/Repositories-Skills/<repo-name>` if it's purely for documentation, analysis, `repodocs`, or `blume`.
  If it requires dependencies, create a dedicated `uv venv` or install them using `uv pip`.

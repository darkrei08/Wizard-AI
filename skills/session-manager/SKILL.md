...
description: "SESSION MANAGER — Dual mode skill (Step 1 PRE e Step 10 POST). Ripristina il contesto all'inizio e salva lo stato persistente alla fine. Utilizza engram come sistema primario, con fallback su MEMORY.md."
---
...
**IMPORTANT**: `engram` is the primary system for persistent MCP memory. This skill MUST defer to `engram` when available.

## Mode 1: RESTORE (PRE-PROMPT - Step 1)

**When:** AUTOMATICALLY before addressing any new user prompt.

1. **Read Memory**: First, query `engram` (e.g. using `mem_context` or `mem_search`). If `engram` is unavailable, silently read `MEMORY.md` (or `CLAUDE.md`) in the project root.
...
2. **Invoke the Memory System**: 
   - **PRIMARY**: Save state to `engram` via `mem_save` or `mem_session_summary`.
   - **FALLBACK**: Only if `engram` is unavailable, execute:
     ```bash
...
If using the fallback `MEMORY.md` and you notice it is getting excessively long (dozens of entries), proactively compress all past entries into a single "Current Project State" document and overwrite the file to keep context lean. If using `engram`, trust its internal lifecycle management.

...
- **GitHub Authentication Failures**: If pushing or releasing fails due to authentication issues (e.g., invalid token, password authentication not supported), you MUST inform the user and ask them to authenticate (using `gh auth login`, a new Personal Access Token, or SSH). Then, provide them with the exact manual commands to finish the publication:
  ```bash
  git push origin main
  git push origin <tag>
  # Optional: generate GitHub release page with changelog
